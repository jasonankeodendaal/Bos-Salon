...
const InvoicePreviewModal: React.FC<{ invoice: Invoice, onClose: () => void }> = ({ invoice, onClose }) => {
    // ... (Existing implementation, omitting for brevity as it was provided) ...
    return (
          ...
          <div className="bg-white/80 backdrop-blur-lg border border-gray-200 rounded-xl shadow-2xl p-8">
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              <div>
                ...
                <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-white border border-gray-300 rounded-lg p-3 text-brand-light outline-none focus:ring-1 focus:ring-brand-gold" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-600 mb-2">Password / PIN</label>
                <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-white border border-gray-300 rounded-lg p-3 text-brand-light outline-none focus:ring-1 focus:ring-brand-gold" />
              </div>
              {error && <p className="text-red-500 text-sm text-center">{error}</p>}
              <button type="submit" className="w-full bg-brand-green text-white py-3 rounded-full font-bold text-lg hover:bg-opacity-90 shadow-md">Log In</button>
            </form>
          </div>
